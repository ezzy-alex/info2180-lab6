...
if ($_SERVER["REQUEST_METHOD"]==="GET"){
  ...
  if(!empty($_GET["query"])){
   ...
   $ans = filter_var($_GET["query"], FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_NO_ENCODE_QUOTES);
    
    $c = 0;
      foreach($superheroes as $v){
      if(($v['name'] === $ans) || ($v['alias'] === $ans)){
  ?>
                <h3><?=$v['alias']?></h3>
                <h4><?php echo "A.K.A " . $v['name'];?></h4>
                <p><?=$v['biography']?></p>
